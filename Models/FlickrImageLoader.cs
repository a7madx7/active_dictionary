...
namespace Active_Dictionary.Models
{
    ...
    public class FlickrImageLoader
        : IContentLoader
    {
        private const string apiKey = "b9f5a293735a53515bf8ce89a8a148ed";
        ...
        public async Task<LinkCollection> GetInterestingnessListAsync()
        {
            if (apiKey == null)
            {
                throw new InvalidOperationException(
                    "You need to specify a Flickr API key. Unfortunately the key cannot be distributed with the source code. Get your own from http://www.flickr.com/services/api/misc.api_keys.html");
            }

            const int count = 50; // limit the number of images to 50
            var listUri = string.Format(CultureInfo.InvariantCulture,
                "http://api.flickr.com/services/rest/?method=flickr.interestingness.getList&api_key={0}&per_page={1}",
                apiKey, count);
            var client = new HttpClient();
